...
void UGnuMyCharacterAnimInstance::UpdateMovementState()
{
	//------------------------------ ShouldMove 확인 ----------------------------
...
void UGnuMyCharacterAnimInstance::UpdateDirectionAndMovementInput()
{
	// ------------------------------ Direction 계산 -----------------------------
...
void UGnuMyCharacterAnimInstance::SetTurnRate(float CurYaw) // GnuCharacterPlayerController에서 호출
{
...
void UGnuMyCharacterAnimInstance::ServerSetAnimState_Implementation() // 클라이언트에서 스프린트 시작 요청을 보내면 서버에서 ServerSprintStart_Implementation이 실행
{
...
bool UGnuMyCharacterAnimInstance::ServerSetAnimState_Validate()
{
	return true; // 필요한 유효성 검사 로직을 여기에 추가
}


void UGnuMyCharacterAnimInstance::ClientSetAnimState_Implementation() // 클라이언트에 즉시 반영하는 방식이지만 서버 - 클라이언트 일관성이 떨어짐
{
...
void UGnuMyCharacterAnimInstance::OnRep_SetAnimState() 	// 클라이언트가 동기화 되는 함수 -> 서버에서 isSprint 값이 변경될 때 클라이언트에서 그 변화를 감지하고 실행되는 함수 (즉, 서버가 관리하는 스프린트 상태를 클라이언트가 동기화 하는 방식)
{
	SetWeapon();
	if (GEngine)
	{
