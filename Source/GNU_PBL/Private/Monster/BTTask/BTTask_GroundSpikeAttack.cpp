...
#include "Monster/BTTask/BTTask_GroundSpikeAttack.h"
#include "Monster/GnuMonster.h"
#include "AIController.h"			// OwnerComp 쓸려고 필요

UBTTask_GroundSpikeAttack::UBTTask_GroundSpikeAttack()
{
	NodeName = TEXT("GroundSpikeAttack");
	// TickTask 활성화
	bNotifyTick = true;
}

EBTNodeResult::Type UBTTask_GroundSpikeAttack::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
void UBTTask_GroundSpikeAttack::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	AGnuMonster* Monster = Cast<AGnuMonster>(OwnerComp.GetAIOwner()->GetPawn());
	if (Monster == nullptr)
	{
